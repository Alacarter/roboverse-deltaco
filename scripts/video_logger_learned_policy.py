...
class BulletVideoLogger:
    ...
    def add_robot_view_to_video(self, images, robot_images):
        ...
        for i in range(len(images)):
            ...
            image_new[
                robot_view_margin:x_offset + robot_view_margin,
    ...
    def collect_traj_and_save_video(self, task_idx, path_idx):
        images = []
        robot_images = []
        obs = self.env.reset()
        ...
        if self.save_all:
            self.save_function(images, robot_images, task_idx, path_idx)
    ...
    def save_video(self, images, robot_images, task_idx, path_idx):
        # Save Video
        save_path_wo_ext = self.get_save_path_wo_ext(task_idx, path_idx)
        save_path = f"{save_path_wo_ext}.mp4"
        if self.add_robot_view:
            dot_idx = save_path.index(".")
            save_path = (
                save_path[:dot_idx] + "_with_robot_view" + save_path[dot_idx:])
        inputdict = {'-r': str(12)}
    ...
    def save_images(self, images, robot_images, task_idx, path_idx):
        # Save Video
    ...
    def get_save_path_wo_ext(self, task_idx, path_idx):
        save_path_wo_ext = (
            ...
            f"{self.env_name}_taskid_{task_idx}_{path_idx}")
